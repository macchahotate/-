...
checkBtn.addEventListener('click', () => {
    ...
    kekkaSound.play()
        .then(() => {
            // 再生が成功したら、3秒後に結果を表示する処理を実行
            
            // 3秒（3000ミリ秒）待ってから結果表示
            setTimeout(() => {
                // 最終結果セクションに切り替え
                ...
                players.forEach(player => {
                    ...
                    listItem.innerHTML = `<strong>${player.name}</strong>: <span class="final-number">${player.number}</span>`;
        ...
        .catch(error => {
            ...
            // エラーが発生しても処理を続行するため、遅延なしで表示
            alert("音声ファイルの再生に失敗しました。すぐに結果を表示します。");
            
            // 遅延なしで即座に最終結果セクションに切り替え
            switchSection(finalSection); 
            finalResultsUl.innerHTML = ''; // リストをクリア
            players.forEach(player => {
                const listItem = document.createElement('li');
